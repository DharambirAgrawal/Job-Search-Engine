  ...
  async buildInvertedIndex() {
    try {
      ...
      jobs.forEach(job => {
        ...
        const skillTokens = job.requiredSkills.flatMap(skill => this.tokenize(skill));
      ...
      console.log(`Inverted index built with ${Object.keys(this.invertedIndex).length} tokens`);
    } catch (error) {
